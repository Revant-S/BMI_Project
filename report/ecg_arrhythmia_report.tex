...
\begin{titlepage}
    ...
    \begin{abstract}
        ...
        classification of cardiac arrhythmias from standard 12-lead ECG signals.
...
The complete processing pipeline is depicted in Figure~\ref{fig:pipeline}.
...
\texttt{
\footnotesize
PhysioNet MIT-BIH (48 records) $\longrightarrow$ [download\_data.py] $\longrightarrow$ \\[2pt]
Beat Extraction (extract\_beats.py) $\longrightarrow$ \\[2pt]
CWT Scalograms 224$\times$224 px (build\_scalograms.py) $\longrightarrow$ \\[2pt]
DataLoaders 70/15/15 split (dataset.py) $\longrightarrow$ \\[2pt]
Model Training (train.py) $\longrightarrow$ Evaluation (generate\_results.py)
}
\end{tabular}
\caption{End-to-end ECG arrhythmia detection pipeline.}
\label{fig:pipeline}
\end{figure}

\subsection{Beat Extraction}

Algorithm~\ref{alg:beat} summarises the beat extraction procedure.

\begin{lstlisting}[caption={Beat extraction from a single MIT-BIH record.},label=alg:beat,language=Python]
for record_id in ALL_RECORDS:
    record  = wfdb.rdrecord(record_id)      # load signal (650,000 x 2)
    ann     = wfdb.rdann(record_id, 'atr')  # load annotation
    ...
    for i, (sample, symbol) in enumerate(zip(ann.sample, ann.symbol)):
        ...
        # Extract fixed window around R-peak
        start = sample - WINDOW_BEFORE       # 90 samples before
        end   = sample + WINDOW_AFTER        # 160 samples after
        if start < 0 or end > len(signal):
            continue
        beats.append(signal[start:end])      # 250-sample window
        labels.append(label)
...
\begin{tabular}{llll}
    \toprule
    \textbf{Block} & \textbf{Operation} & \textbf{Output size} & \textbf{Filters / Units} \\
    \midrule
    Block 1 & Conv3$\times$3 -- BN -- ReLU -- MaxPool & $112\times112$ & 16 \\
    Block 2 & Conv3$\times$3 -- BN -- ReLU -- MaxPool & $56\times56$   & 4 \\
    Block 3 & Conv3$\times$3 -- BN -- ReLU -- MaxPool & $28\times28$   & 8 \\
    Block 4 & Conv3$\times$3 -- BN -- ReLU -- MaxPool & $14\times14$   & 16 \\
...
elements include:
\begin{itemize}[leftmargin=*]
    \item \textbf{Inception modules}: Parallel branches of $1\times1$,
          $3\times3$, $5\times5$ convolutions and max-pooling, concatenated
...
representl.  Nonetheless, its 82.41\% AUC indicates it has learned a
...
Addison, P.~S. (2005).
\textit{Wavelet transforms and the ECG: A review}.
