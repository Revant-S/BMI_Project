"""
Generate all results from the requested checkpoint.
Produces: training curves, confusion matrix, ROC curve, and metrics summary.

Usage:
    python generate_results.py --model smallnet
    python generate_results.py --model googlenet
"""
...
def run_evaluation(model_name):
    ...
    print("=" * 60)
    print(f"  Generating Results for {model_name.upper()}")
    print("=" * 60)

    history_path = RESULTS_DIR / f"history_{model_name}.json"
    epochs_run = 0
    history = None
    if history_path.exists():
        ...
        ax1.set_title(f'{model_name.capitalize()} - Training & Validation Loss', fontsize=13, fontweight='bold')
        ...
        ax2.set_title(f'{model_name.capitalize()} - Training & Validation Accuracy', fontsize=13, fontweight='bold')
        ax2.legend(fontsize=11)
        ax2.grid(True, alpha=0.3)

        fig.tight_layout()
        fig.savefig(RESULTS_DIR / f"training_curves_{model_name}.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    ...
    ckpt_path = RESULTS_DIR / f"best_{model_name}.pth"
    ...
    ckpt_val_acc = checkpoint.get('val_acc', 0)
    print(f"  Loaded checkpoint from epoch {ckpt_epoch} (val_acc={ckpt_val_acc:.2f}%)")
    ...
    with open(RESULTS_DIR / f"metrics_{model_name}.json", 'w') as f:
    ...
    ax.set_title(f'Confusion Matrix - {model_name.capitalize()}', fontsize=14, fontweight='bold')
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / f"confusion_matrix_{model_name}.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    ...
    ax.plot(fpr_roc, tpr_roc, color='#4e79a7', lw=2.5,
            label=f'{model_name.capitalize()} (AUC = {metrics["auc"]/100:.4f})')
    ax.plot([0, 1], [0, 1], color='gray', linestyle='--', lw=1)
    ax.fill_between(fpr_roc, tpr_roc, alpha=0.1, color='#4e79a7')
    ax.set_xlabel('False Positive Rate', fontsize=13)
    ax.set_ylabel('True Positive Rate', fontsize=13)
    ax.set_title(f'ROC Curve - {model_name.capitalize()}', fontsize=14, fontweight='bold')
    ax.legend(loc='lower right', fontsize=12)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / f"roc_curve_{model_name}.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("  [OK] ROC curve saved")

    # Summary dashboard
    fig = plt.figure(figsize=(18, 10))
    fig.suptitle(f'ECG Arrhythmia Detection - {model_name.capitalize()}',
    ...
    info_text = (
        ...
        f"\n"
        f"Epochs completed: {epochs_run}\n"
        f"Best checkpoint: Epoch {ckpt_epoch}\n"
    )
    ...
    fig.savefig(RESULTS_DIR / f"dashboard_{model_name}.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, required=True, choices=['smallnet', 'googlenet', 'all'])
    args = parser.parse_args()

    if args.model == 'all':
        run_evaluation('smallnet')
        run_evaluation('googlenet')
    else:
